...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	rawdb.WriteBlock(bc.db, block)       //保存区块
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		status, err := bc.WriteBlockWithState(block, receipts, state)                  //将区块链里面的区块保存到区块链里面
		if err != nil {
			return i, events, coalescedLogs, err
		}
		switch status {
...
// reorgs takes two blocks, an old chain and a new chain and will reconstruct the blocks and inserts them   新的链总难度大于本地链总难度时，用新的区块链来替换本地的区块链为规范链
// to be part of the new canonical chain and accumulates potential missing transactions and post an
// event about them
//接受两个区块作为参数，一个是老得区块链，一个新的区块链，这个方法会把她们插入；以便重新构建出一条规范的区块链。同时会累计潜在会丢失的交易并把它们作为事件发不出去
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	var (
		...
		//collectlogs会收集我们已经生成的日志信息，这些日志稍后会被声明删除（实际上在数据库中并没有被删除）
	...
	// first reduce whoever is higher bound    //谁产生的数比较大
	if oldBlock.NumberU64() > newBlock.NumberU64() {         //区块高度
		// reduce old chain
		//如果老得链比新的链高，那么需要减少老的链，让它和新链一样高
		...
		// reduce new chain and append new chain blocks for inserting later on     //如果新链比老得链高，那么减少新链
