...
func (tab *Table) closest(target common.Hash, nresults int) *nodesByDistance {
	...
	close := &nodesByDistance{target: target}                    //两节点间距离
	for _, b := range &tab.buckets {
...
func (h *nodesByDistance) push(n *Node, maxElems int) {
	...
	if ix == len(h.entries) {
		...
		// this will overwrite the entry we just appended. （删了一些东西又加了些东西）
		//挨着近的往前靠
		copy(h.entries[ix+1:], h.entries[ix:])         //内存拷贝
		h.entries[ix] = n       //挨着最近的元素
	}
}
