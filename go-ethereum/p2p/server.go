...
type Config struct {
	...
	PrivateKey *ecdsa.PrivateKey `toml:"-"`              //有私钥

	// MaxPeers is the maximum number of peers that can be
	// connected. It must be greater than zero.
	MaxPeers int                                                                     //最大的节点数量 （设置可连接的最大节点数量）

	// MaxPendingPeers is the maximum number of peers that can be pending in the
	// handshake phase, counted separately for inbound and outbound connections.
	// Zero defaults to preset values.
	MaxPendingPeers int `toml:",omitempty"`                                          //最大的交易等待数
	...
	DiscoveryV5 bool `toml:",omitempty"`                                               //发现新节点，找节点
	...
	// Internet.
	NAT nat.Interface `toml:",omitempty"`                          //穿透，一包数据从一个路由器穿透到另一个路由器节点上（实现全网，广域网上的广播）
...
//start主要做了三件事：生成路由表；生成DialState用于驱动维护本地peer的更新与死亡；监听本地接口用于信息应答；（主要分析peer的维护过程，函数为run函数）
func (srv *Server) Start() (err error) {
	...
	srv.quit = make(chan struct{})               //退出节点的时候做了什么     通道
	srv.addpeer = make(chan *conn)
	srv.delpeer = make(chan peerDrop)
	srv.posthandshake = make(chan *conn)
	srv.addstatic = make(chan *enode.Node)         //发现新节点的时候做了什么
	...
	if err := srv.setupDiscovery(); err != nil {                 //生成路由表
		return err
	}

	dynPeers := srv.maxDialedConns()
	//newDialState对象生成，这个对象包含Peer的实际维护代码
...
func (srv *Server) run(dialstate dialer) {
	...
	// removes t from runningTasks           把t任务从   移除
	delTask := func(t task) {
	...
	// starts until max number of active tasks is satisfied       最大数量被激活的任务合适了，便启动任务
